...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, labels, ori_img):
        ...
        detections = [Detection(bbox_tlwh[i], conf, class_id, features[i]) for i,(conf, class_id) in enumerate(zip(confidences, labels))]
        ...
        for track in self.tracker.tracks:
            ...
            outputs.append(np.array([x1,y1,x2,y2,track_id,track.class_ID,track.conf_score], dtype=np.float32))
        if len(outputs) > 0:
